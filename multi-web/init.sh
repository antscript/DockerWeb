...
do
	...
	sed -i "s/container_name/${site_container}/g" /etc/dockerWeb/nginx/conf.d/${site_container}_hook.conf
	...
	then
		...
		sed -i "s#root   /data/DockerWeb/web#proxy_pass https://${site_container}:443#g" /etc/dockerWeb/nginx/conf.d/${site_container}.conf
		...
		sed -i "s#root   /data/DockerWeb/web#proxy_pass http://${site_container}#g" /etc/dockerWeb/nginx/conf.d/${site_container}.conf
	fi
done
